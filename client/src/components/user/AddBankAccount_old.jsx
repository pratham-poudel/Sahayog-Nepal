...
const AddBankAccount = ({ onClose, onSuccess }) => {
  ...
  const steps = [
    ...
    { number: 2, title: 'Document Upload', description: 'Upload verification documents' },
  ...
  const documentTypes = [
    ...
    { 
      ...
      label: 'Passport', 
      required: false,
      description: 'Upload your passport (optional but recommended)',
      icon: DocumentIcon
    }
  ];
  ...
  const validateStep2 = () => {
    ...
    // Check if selected document type has been uploaded
    const selectedDocType = formData.documentType;
    if (!selectedDocType) {
      newErrors.documentType = 'Please select a document type first';
    } else if (!documents[selectedDocType].file && !documents[selectedDocType].uploaded) {
      newErrors[selectedDocType] = `${selectedDocType.charAt(0).toUpperCase() + selectedDocType.slice(1)} document is required`;
    }
  ...
  const handleFileSelect = (documentType, file) => {
    ...
    if (!validTypes.includes(file.type)) {
      setUploadErrors(prev => ({
        ...prev,
        [documentType]: 'Please upload a valid image (JPEG, PNG, GIF) or PDF file'
      }));
      return;
    }
  ...
  const removeFile = (documentType) => {
    ...
    setDocuments(prev => ({
      ...prev,
      [documentType]: { file: null, preview: null, uploaded: false }
    }));
  };
  ...
  const handleSubmit = async () => {
    ...
    setIsSubmitting(true);    try {
      ...
      // Add the selected document type's file as the primary document
      const selectedDocType = formData.documentType;
      if (selectedDocType && documents[selectedDocType] && documents[selectedDocType].file) {
        submitFormData.append('documentImage', documents[selectedDocType].file);
      }

      // Add any additional documents
      Object.keys(documents).forEach(docType => {
        if (documents[docType].file && docType !== selectedDocType) {
          submitFormData.append('additionalDocuments', documents[docType].file);
          submitFormData.append('documentTypes', docType);
        }
      });
    const response = await fetch(`${API_BASE_URL}/api/bank/accounts`, {
      ...
      if (data.success) {
        ...
        if (onSuccess) onSuccess(data.bankAccount);
        if (onClose) onClose();
      } else {
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                    Upload clear, high-quality images of your documents. Ensure all text is readable and the entire document is visible.
                  </p>
                </div>
              </div>
            </div>

            {documentTypes.map(({ key, label, required, description, icon: Icon }) => (
              <div key={key} className="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
                ...
                {documents[key].file || documents[key].uploaded ? (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-900 dark:text-white">
                            {documents[key].file?.name || `${label} uploaded`}
                          </p>
                          <p className="text-xs text-gray-500 dark:text-gray-400">
                            {documents[key].file && `${(documents[key].file.size / 1024 / 1024).toFixed(2)} MB`}
                          </p>
                        </div>
                      </div>
                      <div className="flex space-x-2">
                        {documents[key].preview && (
                          <button
                            type="button"
                            onClick={() => window.open(documents[key].preview)}
                ...
                {uploadErrors[key] && (
                  <p className="mt-2 text-sm text-red-600">{uploadErrors[key]}</p>
                )}
              </div>
            ))}
          </motion.div>
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900 dark:text-white">{formData.accountHolderName}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Branch</p>
                  <p className="font-medium text-gray-900 dark:text-white">{formData.branchName}</p>
                </div>
                {formData.isPrimary && (
