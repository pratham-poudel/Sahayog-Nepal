...
const WithdrawalManagement = () => {
  ...
  const fetchWithdrawals = async (page = 1) => {
    ...
    try {
      ...
      const data = await response.json();      if (data.success) {
        ...
        setStats(data.stats);
        setPagination(prev => ({
          ...(prev || { page: 1, limit: 20, total: 0, totalPages: 0 }),
  ...
  const handleFilterChange = (key, value) => {
    ...
    setPagination(prev => ({ ...(prev || { page: 1, limit: 20, total: 0, totalPages: 0 }), page: 1 }));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
        {Object.entries(stats).map(([status, statData]) => (
          <div key={status} className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{statData?.count || 0}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {formatCurrency(statData?.totalAmount || 0)}
                </p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 px-4 py-3 border-t border-gray-200 dark:border-gray-700 sm:px-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <p className="text-sm text-gray-700 dark:text-gray-300">                  Showing {(((pagination?.page || 1) - 1) * (pagination?.limit || 20)) + 1} to{' '}
